...
## 7. blade-v3-grouped-202608 正式发布

2026-08-04在不修改`blade-v2`的前提下，发布不可变索引版本`blade-v3-grouped-202608`。正式划分为train 33,804张、val 7,244张、test 7,243张，总计48,291张和49,471个实例。

初始`blade-v3-grouped`候选在发布前复核中发现47对同细类相邻拍摄样本跨split，因此未直接发布。修订候选将所有“相邻帧且细类重叠”候选按保守规则闭合并组，再进行组级均衡。最终审计结果为：68个跨旧split SHA组全部字节一致且最终同组；18个文件名前缀仅作为推断字段；94对仍跨split的相邻候选均不共享细类；第二轮48对感知哈希候选复核无未处理跨split重复。

正式版本只保存相对路径清单，图片和标签继续引用冻结的`datasets/blade-v2`，不复制约180GB原图。`test`清单已锁定，只允许在负责人批准后的最终评估中读取，不能用于训练、阈值选择、best epoch或超参数选择。完整说明见`docs/dataset_card_blade_v3.md`。
